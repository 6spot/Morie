...
- Apple on-device candidate extraction from saved History text, with retained input snapshots and explicit review before saving Memory.
...
7. To retain a term or project, open a saved Capture and choose **Find Memory Candidates**, review/edit each suggestion, and save or dismiss it. **Save Memory…** and **Memory → New Memory** also support manual entries. Pending suggestions appear in Memory; only confirmed active memories participate in context retrieval.

Candidate extraction uses saved final text and retains the exact input used. Once M-005 adds AI polishing, its saved polished output will feed this boundary while recognized text remains available. Polishing and input correction are not implemented yet.
