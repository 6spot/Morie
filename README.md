...
- local SwiftData Capture-first persistence and a unified native management window for History, Memory, Settings, and Diagnostics;
- side-by-side History/Memory browsing with search and filters, readable details, native recording playback, cancellable re-recognition, and explicit copy/delete actions;
...
Phase 0 runtime validation, Phase 1 Capture persistence, Phase 2 Memory, the first Phase 3 personalization slice and M-008 management UI acceptance are in progress. Development is macOS-only; iOS is not currently scheduled. The owner deferred interactive device validation until the evening of 2026-09-18 while development continues. iCloud/CloudKit sync remains deferred to final integration after Apple Developer enrollment.
...
8. Select a Capture to read **Final Text** beside the list. Expand **Recognition & Refinement** for the recognized text and refinement record, or **Source Recording** for playback/retry. **Settings → Use Memory to Refine Input** controls optional refinement and defaults to enabled.
