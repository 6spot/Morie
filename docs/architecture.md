...
`InputRefinementRunner` owns one model task, one continuation and a foreground wait deadline. Local/automatic cleanup may wait up to 12 seconds and explicit cloud cleanup up to 20 seconds before the Capture falls back to the prepared transcript with `.timeLimit`; caller cancellation releases the Capture immediately as well. Neither path joins a model task that ignores cancellation, so a late result is ignored while the owned task drains. Processing remains user-cancellable until delivery begins. Background Memory learning is preempted at Capture start and no longer causes foreground cleanup to be skipped merely because its cancelled model call is still unwinding.
