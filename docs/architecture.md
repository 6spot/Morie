...
Speech and refinement now share the same bounded canonical Dictionary set (up to 100 entries / 2,000 characters). ASR uses it as native hints; refinement receives it as read-only reference data and the model decides semantic relevance. Morie does not hide terms from refinement based on transcript edit distance. Confirmed correction rules are still loaded only when the observed form exists, applied deterministically before cleanup and retained as provenance/staleness data; they are not serialized into the model prompt.
...
The capture request is frozen at Start; the asynchronous result is accepted only if that Capture is still active, retained in runtime memory, and released with session identity reset. It is intentionally not `Codable` and is never attached to `CaptureRecord`, `CaptureRefinement`, History or Personal Memory. Bounded high-signal vocabulary extracted from selected/focused/nearby text can bias both live and saved-audio Apple Speech through `AnalysisContext`. The same bounded term set may reach cleanup as runtime-only read-only reference data; the model decides relevance. Raw Application Context text remains runtime-only and never becomes an instruction surface.
...
Source-audio evidence remains an independent local heuristic used only after recognition to decide whether a completely empty Capture can be discarded or should stay retryable. It is not allowed to suppress analyzer finalization or the saved-audio retry when recorded frames exist. Apple `SpeechDetector` is deliberately excluded from both live and saved-audio analyzer module lists because it can gate transcription and drop speech; Morie relies on the transcriber result plus its own post-recognition evidence instead. Quiet-speech and room-noise thresholds remain real-device acceptance items.
...
`CaptureFileTranscriber` uses the same native backend preference as live input: `SpeechTranscriber(.transcription)` when the locale/device supports it, otherwise `DictationTranscriber(.longDictation)`. It reads an existing `AVAudioFile` with `SpeechAnalyzer.analyzeSequence(from:)`, finalizes through the consumed audio, preserves Apple's segment whitespace, and closes native analysis on cancellation or failure. Normal Capture completion calls this path once after live finalization and supplies the session-frozen Dictionary plus Application Context Speech hints through `AnalysisContext`. A non-empty saved-audio result replaces the live transcript only when its semantic character count is not a gross truncation/expansion regression. Explicit History re-recognition does not reconstruct historical hint state.
...
`Speech → durable recognized text → dictionary corrections → model cleanup with bounded reference data → lifecycle/payload boundary → durable final text → delivery`
...
`InputRefiner` creates a fresh Apple `LanguageModelSession`, supplies transcript/dictionary/context as JSON data under the [approved cleanup instructions](input-cleanup.md), and requests complete final text through greedy `@Generable` output. The shipped default instruction is a three-paragraph text resource, `DefaultRefinementInstructions.txt`. `RefinementPromptController` loads the effective instruction into process memory once; Settings edits replace that in-memory value, while `UserDefaults` only restores the override on a future launch. The live cleanup path never rereads either storage source. `RefinementConfiguration` freezes that effective instruction together with the selected model at Capture Start, while external credentials continue to resolve lazily only when refinement actually runs. Native token accounting bounds the full prompt, instructions, schema and a response budget of 256–1,536 tokens. Oversized input is declined without truncating the saved text.

Foundation Models owns semantic cleanup and contextual correction through the approved instructions and `@Generable` structured result. Speech keeps the full bounded hint set; cleanup receives the bounded Application Context term set as read-only reference data and the model decides relevance. Confirmed Dictionary mappings remain deterministic pre-model edits rather than prompt material. Morie no longer reimplements semantic validation with negation lists, correction phrases, number/time regexes, URL/path comparisons, script checks or output-length ratios. The post-generation boundary only rejects unusable empty/control-character payloads; snapshot freshness, Dictionary/Memory/style changes, cancellation, timeout and durable-save failure remain separate code-owned consistency checks.

`InputRefinementRunner` owns one foreground model task, one continuation and a foreground wait deadline. Local cleanup may wait up to 12 seconds; automatic/cloud cleanup may wait up to 20 seconds before the Capture falls back to the prepared transcript with `.timeLimit`; caller cancellation releases the Capture immediately as well. A provider task that ignores cancellation is quarantined as draining work so its late result is ignored **and it does not block the next Capture from starting refinement**. Processing remains user-cancellable until delivery begins. Background Memory learning is preempted at Capture start and no longer causes foreground cleanup to be skipped merely because its cancelled model call is still unwinding.
...
The refinement prompt follows a **data-boundary contract** rather than a second semantic rule engine. `InputRefiner.trustedSystemBoundary` is non-editable and is appended after the user-editable cleanup instruction for both Apple-local and cloud refinement, so an old/custom prompt cannot remove the authority separation. `transcript` is the primary user-authored material; Dictionary spellings, bounded Application Context terms, topic-level Memory hints and Expression Profile directives are explicitly read-only reference data. The model may use those references when its language understanding says they resolve ASR/context ambiguity, but they cannot become instructions or grant capabilities. M-035 removed deterministic formatting routing; this revision also removes the post-generation semantic output validator. The local `@Guide` remains schema-only.

### `MemoryRecord` / `MemoryEvidenceRecord` / `MemoryStore`

M-036 changes Personal Memory from a flat fact list into a small semantic topic/entity store. `MemoryRecord` is the current user-visible body for one topic. It keeps a stable record ID while an automatic topic is merged or updated; generated display names are no longer treated as semantic identity. The internal `MemoryScope` is either `longTerm` or `workingContext`, but that taxonomy is not exposed as a management category.

`MemoryEvidenceRecord` persists evidence separately from the current topic body: exact saved final text, supporting source quote, Capture ID, evidence time and optional model confidence. This lets one topic accumulate or change while its provenance remains inspectable. Deleting a source Capture removes its analysis record but does not erase the already-independent Memory evidence snapshot.

Code, not the language model, owns lifecycle policy. Long-term Memory does not expire automatically. Working context receives a 30-day expiry from its latest evidence; reinforcement refreshes that deadline, and a model-proposed merge/update may promote it to long-term by removing expiry. Expired working context is archived internally rather than treated as an explicit user rejection. User edits become durable user-owned Memory and cannot be overwritten automatically.

Explicit user archive/delete is stronger than automatic learning. User-archived topics and deleted-topic tombstones are supplied to the writer model as protected context. A small structural guard remains as a final deterministic defense, but generated topic-name equality is no longer the automatic semantic key.

Personal Memory defaults on and has an independent native Settings toggle. While off, completed current-app Captures receive a durable skipped analysis marker so turning Memory back on cannot silently backfill input recorded during the disabled period. Existing Memory/evidence is retained for inspection, and cleanup receives no Personal Memory context.

### `MemoryContextRetriever`

The cleanup-side retriever remains deliberately unchanged in M-036 except for the global Memory switch. It consumes active immutable Memory snapshots and ranks bounded lexical name/content overlap, capped at eight. This is still local deterministic retrieval, not semantic recall. M-036 first fixes what Memory means and how it is maintained; semantic cleanup retrieval and broader prompt-use policy are the explicit follow-up.

### `MemoryAnalysisRecord` / `MemoryLearner` / `MemoryLearningController`

Completed `currentApp` Captures (`delivered` or `deliveryFailed`) with committed final text are durable queue sources. Active, cancelled, capture-only, running-refinement and raw-only inputs do not enter learning. Normal completion enqueues only that Capture; startup reconciliation discovers current-schema completed work that may have been saved just before a crash.

The writer remains Apple-local in M-036. `MemoryLearner` uses a fresh `SystemLanguageModel.default` session and receives the exact source, up to 24 recent active semantic Memory snapshots, and bounded protected user-archived/deleted topics. It proposes at most three changes. The model owns semantic classification: long-term versus working context, semantic existing-topic identity by supplied UUID, and `create / merge / update / reinforce`. It may synthesize a concise current topic body from supplied context/evidence, but every proposed change must include an exact supporting quote from the current source.

Local admission no longer tries to reproduce semantic understanding with pronoun lists, update keywords, exact repeated drafts or generated-name keys. It verifies source freshness, literal evidence grounding, valid supplied UUID/snapshot identity, user-owned-body protection and atomic persistence. Automatic merge/update mutates the existing semantic topic in place; reinforce adds evidence without rewriting the current body. Analysis completion, topic mutation and evidence insertion share one save.

The controller remains event-driven and input-preemptible. It waits for input idle time, processes at most three sources per batch, backs off retryable failures, and immediately yields to new voice input. Turning Personal Memory off cancels optional work; disabled-period Captures are recorded as skipped rather than queued for later surprise learning. A late cancelled result cannot commit.

M-036 deliberately does not reuse the user-configured external refinement provider for Memory learning. Configuring a cleanup API therefore cannot silently send the user's complete daily Memory-learning source to that provider. A future provider choice for Memory would require its own explicit product/privacy surface.

### Native Memory management

`MemoryView` presents a natural flat reading surface: topic title, current body and inspectable source/evidence history. It does not expose `longTerm / workingContext` or internal kind taxonomy as user-managed categories. Manual create/edit asks only for a topic and content; that user-authored Memory is durable and takes precedence over later automatic learning. Archive/restore/replace/delete remain optional native management actions; there is no candidate inbox or required review task.

`MemoryLearningView` shows per-Capture learning progress/outcomes and the exact saved text used for analysis. Opening or closing History/Memory UI does not govern background learning.
...
Memory tests cover topic/evidence provenance, lifecycle and bounded retrieval. Learning tests inject semantic writer decisions to verify exact final-text snapshots, restart discovery, disabled-period skip semantics, merge/update by existing UUID, working-context expiry/refresh/promotion, user precedence, protected archive/delete, atomic failure/retry, source validation and immediate input preemption. Personalization tests inject model output, including tasks that ignore cancellation, to establish durable ordering, dictionary fallback, cleanup guards, stale-context rejection, deadline/cancellation and recovery. Actual model fidelity, AX/cross-app behavior, UI interaction and latency remain device acceptance.
...
MemoryRecord → current semantic topic/entity body
MemoryEvidenceRecord → exact supporting source/evidence
```
