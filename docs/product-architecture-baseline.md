...
| Personal Memory | Default-on and user-disableable. During idle time, the Apple on-device model maintains a bounded set of semantic topics/entities from committed final daily input, classifying stable long-term information separately from temporary working context. No required candidate-review inbox. |
| Evidence and user control | The current semantic topic body is separate from exact supporting evidence. Code owns source freshness, atomic persistence, working-context expiry/refresh, and user precedence; the model owns semantic create/merge/update/reinforce decisions. User edits, archive and delete take precedence, and deleted/user-archived topics are protected from automatic recreation. |
| Future input | Relevant Memory helps interpret what was said; it cannot insert unspoken background or override the current viewpoint/style. Turning Personal Memory off stops both new learning and Memory injection while retaining saved data. Dictionary and cleanup remain useful independently. |
