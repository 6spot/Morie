...
8. **macOS development focus (2026-09-18):** the owner has no current plan to start iOS. Finish the current macOS foundation, then improve the History/Memory/Settings/Diagnostics management pages with a unified native design language. Reconsider iOS only after the macOS product works reliably; it is not the automatic next task after M-005.
