...
The persistent detail host owns the primary navigation title and the one top-level toolbar declaration. Top-level routed pages must not install their own `.toolbar`, `.searchable`, `.navigationTitle` or other NavigationStack chrome because changing those preferences during sidebar routing causes the window toolbar and sidebar toggle to be recomputed. Search fields, filters and actions remain separate native toolbar items owned by the host; route changes replace the contents of stable toolbar slots rather than visually fusing unrelated controls into one custom surface. Route presentation state is shared with the child page through bindings. Secondary navigation destinations may own their own title/actions after an explicit push.
