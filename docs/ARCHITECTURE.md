...
The persistent detail host owns the primary navigation title and the one top-level toolbar. Top-level routed pages must not install their own `.toolbar`, `.searchable`, `.navigationTitle` or other NavigationStack chrome because changing those preferences during sidebar routing causes the window toolbar and sidebar toggle to be recomputed. Search fields, filters and route actions are composed inside one persistent AppKit toolbar item with reserved geometry; route changes replace only controls inside that slot. Route presentation state is shared with the child page through bindings. Secondary navigation destinations may own their own title/actions after an explicit push.
