...
| Microphone/Speech authorization | DONE / VERIFY | Native first-request prompts plus modal/menu System Settings recovery when macOS returns denial without presenting consent. Only Accessibility suppresses Morie's modal to avoid stacking it over Device Control and Data Access. The app target uses the owner's Apple Development Team so Xcode builds have a stable TCC identity; microphone revoke/re-enable/recheck needs another real-device pass. |
