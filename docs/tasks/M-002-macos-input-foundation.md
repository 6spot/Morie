...
| Microphone/Speech authorization | DONE / VERIFY | Native first-request prompts plus persistent menu/alert System Settings recovery after denial. The target now selects the owner's Apple Development Team so Xcode runs have a stable signing/TCC identity; microphone revoke/re-enable/recheck needs another clean real-device pass. |
...
- a 2026-09-17 Xcode test run later produced capture sessions pinned at `-758.6 dB` and empty transcripts after the repository build directory had also been overwritten by unsigned command-line builds. A later audit found that unsigned temporary products had also been registered with Launch Services while an obsolete repository `build/Debug/Morie.app` remained launchable under a different ad-hoc identity. Those stale paths were unregistered, the obsolete process was stopped, and the target was bound to the owner's Apple Development Team. The owner approved deferring the audio case unless it reproduces from a clean, Xcode-signed run. Automated agent builds must use isolated temporary DerivedData, must not overwrite the locally running Xcode product, and must not leave unsigned products registered with Launch Services.
