...
| Microphone/Speech authorization | DONE / VERIFY | Native first-request prompts plus persistent menu status/System Settings recovery after denial. Morie suppresses its own modal alert for permission-backed failures so it never stacks a second prompt over macOS; microphone revoke/re-enable/recheck needs another real-device pass. |
