...
- A complete local backup was verified at `~/Library/Application Support/Morie/DevelopmentBackups/20260918-170133`: **26 Captures and 9 recordings**, database integrity `ok`, recording checksums matched. An empty current-schema store was prepared and opened successfully in isolation. The owner's active database and signed app were not replaced. Switching the active development store awaits the owner's confirmation because it clears visible History while preserving the backup.

## Known issues / deferred acceptance

- The owner's existing development database contains an incompatible refinement value. Use the explicit, backed-up [development-data reset procedure](../development.md#development-data-after-schema-changes) before its next current-schema launch; do not introduce legacy migrations or erase it automatically.
