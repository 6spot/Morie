...
/usr/bin/plutil -insert configuration -string "${CONFIGURATION:-unknown}" "$OUTPUT_FILE"
/usr/bin/plutil -insert sdk -string "${SDK_NAME:-unknown}" "$OUTPUT_FILE"
/usr/bin/plutil -insert archs -string "${ARCHS:-unknown}" "$OUTPUT_FILE"
/usr/bin/plutil -insert xcodeVersion -string "${XCODE_VERSION_ACTUAL:-unknown}" "$OUTPUT_FILE"
