...
extension ValidatedRefinement {
    /// The model owns semantic interpretation of the dictated text. Morie only
    /// validates the transport boundary here: a cleanup result must be usable
    /// text. Meaning, facts, self-corrections, numbers, language, structure and
    /// context relevance are deliberately not reimplemented with local rules.
    static func accepting(
        ...
        guard !output.isEmpty,
            ...
            DevelopmentDiagnostics.record(
                ...
                level: .warning,
                "rejectedInvalidPayload; outputCharacters=\(output.count)"
            )
            throw RefinementReason.invalidEdits
        }

        if output == prepared.text {
            return prepared
        }
        ...
        DevelopmentDiagnostics.record(
            "RefinementBoundary",
            captureID: input.captureID,
            "acceptedModelText; sourceCharacters=\(prepared.text.count); outputCharacters=\(output.count)"
        )

        let cleanup = RefinementEdit(
            original: prepared.text,
            replacement: output
        )
        return ValidatedRefinement(
            text: output,
            edits: prepared.edits + [cleanup]
        )
    }
}
